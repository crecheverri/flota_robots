...
//The FollowJointTrajectory action is used to define a trajectory (as well as tolerance parameters) for the arm structure
//A FollowJointTrajectory action client (called robotClient) is created using the SimpleActionclient template class instatiated for the control_msgs::FollowJointTrajectory
//This action cliant sends the trajectory through a control_msgs::FollowJointTrajectoryGoal to the FollowJointTrajectory action server, that is available because a controller has been defined in a yaml configuration file and spawned in a launch file.
...
int main(int argc, char **argv) 
{
  ...
  ros::init(argc, argv, "JointTrajectory_action_client");
  ros::NodeHandle nh;
  
  //Define simple action client and wait for server
  actionlib::SimpleActionClient<control_msgs::FollowJointTrajectoryAction> robotClient("/nebulizador_arm/arm_joint_trajectory_controller/follow_joint_trajectory");
  ...
  goalTraj.trajectory.joint_names[0] = "link1_joint";
  goalTraj.trajectory.joint_names[1] = "link2_joint";
